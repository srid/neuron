...
renderRouteHead config route val = do
  ...
  elAttr "link" ("rel" =: "shortcut icon" <> "href" =: "https://raw.githubusercontent.com/srid/neuron/new-logo/assets/neuron.svg") blank
  case route of
    Route_Redirect _ ->
      blank
    Route_Search {} -> do
    ...
    divClass "center aligned column" $ do
      divClass "ui tiny image" $ do
        elAttr "a" ("href" =: "https://neuron.zettel.page") $ do
          elAttr
            "img"
            ( "src" =: "https://raw.githubusercontent.com/srid/neuron/new-logo/assets/neuron.svg"
                <> "alt" =: "logo"
                <> "title" =: ("Generated by Neuron " <> neuronVersion)
            )
            blank
